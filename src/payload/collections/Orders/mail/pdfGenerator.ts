...
export const generateOrderPDF = async (order: Order): Promise<Uint8Array> => {
  ...
  const billingAddress = [
    ...
    `State: ${typeof order.orderedBy === 'object' ? stateMap[order.orderedBy.state] + ' (' + order.orderedBy.state + ')' : 'N/A'}`,
...
yOffset -= 15;
const tableHeaders = ['Item Description', 'QN', 'Size', 'Price/unit', 'CGST', 'SGST', 'IGST', 'Total/unit'];
...
const totalPrice = order.total ? (order.total / 100).toFixed(2) + ' INR' : 'N/A';
  ...
  const paymentMode = order.paymentMode ? order.paymentMode : 'N/A';
  ...
  // Footer
  yOffset -= 20;
  firstPage.drawText('For any questions, please contact us at +91 9971643446 or merphpit@gmail.com', {
    x: margin,
    y: yOffset,
    size: fontSize,
    font: helveticaFont,
